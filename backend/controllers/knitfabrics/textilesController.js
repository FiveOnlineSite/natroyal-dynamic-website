...
const updateTextile = async (req, res) => {
  try {
    ...
    const hasContent = content && content.trim() !== "";
const hasLamination = lamination_content && lamination_content.trim() !== "";
const hasCoating = coating_content && coating_content.trim() !== "";

if (hasContent) {
  // clear lamination & coating if content is provided
  updatedFields.content = content;
  updatedFields.lamination_content = "";
  updatedFields.coating_content = "";
} else if (hasLamination || hasCoating) {
  // clear content if lamination or coating is provided
  updatedFields.content = "";
  if (hasLamination) updatedFields.lamination_content = lamination_content;
  if (hasCoating) updatedFields.coating_content = coating_content;
} else {
  // if no content is passed, keep existing values
  updatedFields.content = currentTextile.content;
  updatedFields.lamination_content = currentTextile.lamination_content;
  updatedFields.coating_content = currentTextile.coating_content;
}
