...
const updateVinylAppContent = async (req, res) => {
  try {
    ...
    if (!currentVinylAppContent) {
      return res.status(404).json({ message: "VinylAppContent content not found." });
    }
...
const getVinylAppContent = async (req, res) => {
  try {
    const appContent = await VinylAppContentModel.findById(req.params._id).populate("application", "name");;
...
const getVinylAppContents = async (req, res) => {
  try {
    const VinylAppContent = await VinylAppContentModel.find().populate("application", "name");;
...
const deleteVinylAppContent = async (req, res) => {
  try {
    ...
    const deletedVinylAppContent = await VinylAppContentModel.findByIdAndDelete(VinylAppContentContent._id);
