...
const createVinylApp = async (req, res) => {
  try {
    ...
    const existingApp = await VinylApplicationModel.findOne({ name: name.trim() });
    if (existingApp) {
    ...
    if (req.files && req.files.image && req.files.image[0]) {
      ...
      if (!alt || alt.trim() === "") {
        return res.status(400).json({ message: "Alt text is required for image." });
      }
    ...
    if (req.files && req.files.icon && req.files.icon[0]) {
      ...
      if (!icon_alt || icon_alt.trim() === "") {
        return res.status(400).json({ message: "Alt text is required for icon." });
      }
    ...
    return res.status(500).json({ message: `Error creating vinyl application: ${error.message}` });
  }
};
...
const updateVinylApp = async (req, res) => {
  try {
    ...
    if (name) {
      ...
      if (duplicate) {
        return res.status(400).json({ message: "Another application with this name already exists." });
      }
    }
